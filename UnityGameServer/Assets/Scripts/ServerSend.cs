...
public class ServerSend
{
    ...
    /// <summary>Sends a player's updated camera rotation to all clients except to himself (to avoid overwriting the local player's rotation).</summary>
    /// <param name="_player">The player whose rotation to update.</param>
    public static void CameraRotation(int _playerId, Quaternion _rotation)
    {
    ...
    public static void BulletDestroyed(Bullet _bullet)
    {
        using (Packet _packet = new Packet((int)ServerPackets.bulletDestroyed))
        {
            _packet.Write(_bullet.id);
            _packet.Write(_bullet.transform.position);
