...
public class ClientHandle : MonoBehaviour
{
    ...
    {
        Debug.Log($"Received a player rotation");
        int _id = _packet.ReadInt();
        Quaternion _rotation = _packet.ReadQuaternion();
    ...
    public static void CameraRotation(Packet _packet)
    {
        ...
        if (GameManager.players.TryGetValue(_id, out PlayerManager _player))
        {
            _player.gun.transform.localRotation = _rotation;
        }
    }
    ...
    public static void BulletDestroyed(Packet _packet)
    {
        int _bulletId = _packet.ReadInt();
        Vector3 _position = _packet.ReadVector3();

        GameManager.bullets[_bulletId].Delete();
    }
